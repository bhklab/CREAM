#' ElementRecog_freecutoff is a function to identify LOCKs
#'
...
#' @export
ElementRecog_freecutoff <- function(InputData, windowSize_Vec, peakNumMax, peakNumMin){
  ...
  for(chrIter in ChrSeq){
    ...
    for(peakNumIter in seq(peakNumMax, peakNumMin, by = -1)){
      ...
      while(i < (length(InputData_Start)-(peakNumIter - 1))){
        ...
        Coverage <- sum(as.numeric(InputData_End[i:(i+(peakNumIter - 1))]) -
                          as.numeric(InputData_Start[i:(i+(peakNumIter - 1))]))
        ...
        if(checkwindow < WindowSize){
          ...
          WidthElement_Vec  <- c(WidthElement_Vec, Coverage) #widthElement
